...
如果空白区域偏长，请选择“收纳范围 → 标准”或“紧凑”；如果收起后仍有少量图标可见，再切换为“扩展”。连接更宽显示器、需要准备更多安全状态项时，StatusPerch 会提示重新启动，并保留现有设置。

更新后按住 Command，从左到右重新排列一次：“待收纳图标 → 小竖线 → StatusPerch 箭头 → 时钟/常用图标”。收起后，待收纳图标会进入 macOS 27 的系统 `«` 溢出区域。`«` 是 macOS 控件，并不是 StatusPerch 的图标。

系统 `«` 控件以及它与 StatusPerch 箭头之间的空白区域属于正常表现：StatusPerch 使用受限宽度的原生状态项占据这段空间，在不申请屏幕录制、辅助功能权限或调用非公开接口的情况下，将选定图标推入系统溢出区。请把时钟、电量、Wi-Fi 和其他需要常驻的图标放到 StatusPerch 箭头右侧。

如果连接宽度大很多的显示器后收纳不完整，请接受 StatusPerch 的重新启动提示，让它按照新的显示器布局创建合适数量的安全间隔项。
